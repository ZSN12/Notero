...
from tests.harness.helpers import (
    auth_headers,
    create_notebook_session_note,
    create_other_user,
)


DEFAULT_NOTE_CONTENT = "## 语音转文字\n\n设计模式是软件工程中常用的解决方案。单例模式确保一个类只有一个实例。观察者模式用于对象间的一对多依赖关系。\n\n---\n\n随堂笔记：今天学习了设计模式的基本概念。"
...
def _default_note(client: TestClient, headers: dict[str, str]):
    """Create the standard notebook + session + note used by most vector tests."""
    return create_notebook_session_note(
        client,
        headers,
        title="Vector Test NB",
        session_title="Vector Test Session",
...
def test_rebuild_session_index(client: TestClient):
    headers = auth_headers(client)
    _, session_id = _default_note(client, headers)
...
def test_rebuild_replaces_old_chunks(client: TestClient):
    headers = auth_headers(client)
    _, session_id = _default_note(client, headers)
...
def test_search_returns_relevant_results(client: TestClient):
    headers = auth_headers(client)
    _, session_id = _default_note(client, headers)
...
def test_search_with_session_filter(client: TestClient):
    headers = auth_headers(client)
    _, session_id = _default_note(client, headers)
...
def test_cannot_index_others_session(client: TestClient, db):
    headers = auth_headers(client)
    _, session_id = _default_note(client, headers)

    create_other_user(db, "other_vec@example.com", "Other", "other1234")
    other_headers = auth_headers(client, "other_vec@example.com", "other1234")
...
def test_cannot_search_others_content(client: TestClient, db):
    headers = auth_headers(client)
    _, session_id = _default_note(client, headers)

    client.post(f"/api/vector/session/{session_id}/rebuild", headers=headers)

    create_other_user(db, "other2_vec@example.com", "Other2", "other21234")
    other_headers = auth_headers(client, "other2_vec@example.com", "other21234")
...
def test_rebuild_notebook_index(client: TestClient):
    headers = auth_headers(client)
    notebook_id, _ = _default_note(client, headers)
...
def test_status_indexed_after_rebuild(client: TestClient):
...
def test_status_stale_after_content_change(client: TestClient):
...
def test_status_back_to_indexed_after_rebuild(client: TestClient):
...
def test_status_stale_after_layout_blocks_change(client: TestClient):
...
def test_empty_note_status(client: TestClient):
