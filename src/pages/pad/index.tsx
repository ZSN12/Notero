...
import { deleteAudio, insertPPTIntoTranscript, getMediaUrl, fetchNotebookDetail, fetchSessionById, type Slide } from '@/services/api';
...
import { useTranscript, StudentNote } from '@/pages/note-detail/useTranscript';
...
import { useAudioUpload } from '@/pages/note-detail/hooks/useAudioUpload';
import { useAutoGenerate } from '@/pages/note-detail/hooks/useAutoGenerate';
import { useRestructure } from '@/pages/note-detail/hooks/useRestructure';
import { useProcessingStatus } from '@/pages/note-detail/hooks/useProcessingStatus';
import MindMapCanvas, { computeDefaultExpanded } from '@/pages/note-detail/MindMapCanvas';
import type { ContentBlock, RAGSource } from '@/services/api';

interface WorkflowStatus {
  tone: 'blue' | 'amber' | 'red' | 'violet' | 'green' | 'slate';
  text: string;
...
export default function NoteDetail() {
  ...
  const workflowStatus = useMemo<WorkflowStatus>(() => {
    ...
    if (audioUpload.state.isUploadingAudio || stages?.upload_transcribe?.status === 'running') {
      ...
      return { tone: 'blue', text: '正在处理录音文件', progress: audioUpload.state.audioQueueProgress || undefined };
    }
      ...
      <div className={`flex-shrink-0 mx-4 mt-3 px-3 py-2 border rounded-xl flex items-center gap-2 text-xs ${statusClass}`}>
        ...
        {workflowStatus.progress && workflowStatus.progress.total > 1 && (
          <div className="flex items-center gap-2 w-28 sm:w-36">
            ...
            <span className="text-xs tabular-nums opacity-90">{workflowStatus.progress.current}/{workflowStatus.progress.total}</span>
          </div>
        )}
        {workflowStatus.retryAgents && workflowStatus.retryAgents.length > 0 && (
          <button
            onClick={() => autoGen.actions.handleTriggerAgents(sessionId, workflowStatus.retryAgents)}
